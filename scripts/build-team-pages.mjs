...
import { TEAMS, ISO, slugOf, lookupTeam } from '../lib/teams.mjs';
...
// code → { code, key, he, iso, flag, group, slug }
function info(code) {
  const t = feed.teams[code];
  return { code, key: t.key, he: t.he, iso: t.iso, flag: t.flag, group: groupByCode[code] || '', slug: slugOf(code) };
}
...
const flagChip = (iso, he, cls = 'wc-flag', flag, size = 26) =>
  `<span class="${cls}"${flag ? ` style="background:linear-gradient(135deg,${flag[0]},${flag[1]})"` : ''}><img src="${FLAG_CDN}/${iso}.svg" alt="${esc(he)}" width="${size}" height="${size}" loading="lazy" decoding="async"></span>`;
...
/* ════════════════════════ SNIPPET BUILDERS ════════════════════════ */
const teams = HAS_JERSEY.map(info).filter((t) => t.slug && t.he);
...
function buildHeroBody() {
  ...
  for (const t of teams) {
    ...
    out += `  <span class="wc26t__emblem" style="background:linear-gradient(135deg,${t.flag[0]},${t.flag[1]})"><img src="${FLAG_CDN}/${t.iso}.svg" alt="${esc(t.he)}" width="120" height="120" loading="eager" fetchpriority="high" decoding="async"></span>\n`;
...
function buildInfoBody() {
  ...
  for (const t of teams) {
    ...
    out += `<p class="wc26ti__lead">${esc(copy.infoPara(t.he, t.group, oppList))}</p>\n`;
...
function buildFaqBody() {
  ...
  for (const t of teams) {
    ...
    const qa = copy.faq(t.he, t.group, oppList, fixturesList);
    out += `{%- when '${t.slug}' -%}\n`;
