...
2. **רענון ה-feed רץ בענן** — GitHub Actions + **Pinger אמין (cron-job.org) כל 5 דק', אומת חי**. 24/7, לא תלוי במחשב/שינה.
3. תוקני באגים חיים: חיתוך שם נבחרת ("אוסטרליה") + סקשן "משחקי היום" strict ליום-לוח ישראלי.

> ▶️ **כשתחזור — 3 פריטים פתוחים (לפי עדיפות):**
> 1. **🔵 החלטה: מקור נתונים** — לשדרג ל-API-Football Pro לתוצאות בזמן אמת (openfootball איטי). פרטים מלאים ב"מה נשאר" פריט 2. *(Apify 365scores scraper נבדק ונפסל.)*
> 2. **קישור תפריט** "לוח משחקים" תחת "מונדיאל 2026" (ידני ב-Admin).
> 3. (לא חוסם) Theme Access token ל-SSR, פרסום 30 קולקציות `wc2026-*`.
...
2. ✅ **אמינות ה-cron — בוצע ואומת.** ה-`schedule` המובנה של GitHub לא אמין (ירה פעמיים ב-~5 שעות), אז הוקם **Pinger ב-cron-job.org** שמפעיל את ה-workflow כל 5 דק' דרך GitHub API. **אומת חי 14/6: ריצות `workflow_dispatch` כל ~5 דק' (08:43/08:50/08:55/09:00, כולן success).** מדריך: `SETUP-feed-pinger-cron-job-org.md`. ה-PAT שמור ב-`.env` כ-`FEED_DISPATCH_PAT` (gitignored). 🔒 לרוטט בהזדמנות (היה גלוי בצ'אט) — לעדכן ב-`.env` וב-cron-job.org.

   > **🔵 החלטה פתוחה מרכזית — מקור הנתונים (תוצאות בזמן אמת):** ה-feed מתרענן עכשיו אמין כל 5 דק', אבל המקור **openfootball איטי** (תוצאות באיחור שעות; 14/6 רק 7/104 משחקים עם תוצאה — אוסטרליה–טורקיה הסתיים ~3ש' ולא פורסם שם). ה-pipeline תקין; זה מגבלת מקור.
   > **הומלץ: לשדרג ל-API-Football Pro (~€19/חודש)** — real-time, אמין, מורשה, המנוע כבר בנוי. צעדים: (1) לפתוח חשבון Pro ב-api-football.com, (2) להוסיף את המפתח (כ-repo secret `APIFOOTBALL_KEY` + `LIVE_API=on`), (3) Claude ישפר את `lib/sources.mjs getLive` שישלוף גם משחקים שהסתיימו (לא רק `live=all`), (4) לבדוק שתוצאות נכנסות תוך 1–5 דק'.
   > **נבדק ונפסל (14/6): Apify 365scores-scraper** (apify.com/crawlerbros/365scores-scraper) — בטוח למחשב (קריאות API, בלי התקנה), אבל **לא לפרודקשן**: actor קהילתי עם **1 user / 0 reviews** (שביר, ייתכן ננטש/יישבר אם 365scores ישתנה), **scraping = אפור ToS** לאתר מסחרי, ועלות per-result (~$50–200/חודש בקצב 5 דק') > API-Football. היתרון היחיד (נתוני 365scores) לא שווה את הסיכון.
