...
2. **⚠️ אמינות ה-cron — לפעולה:** ה-`schedule` המובנה של GitHub אומת שעובד אבל **לא אמין** (ירה פעמיים ב-~5 שעות — GitHub חונק `*/5`). **ההחלטה: Pinger חיצוני.** הוקם מדריך מלא ומאומת → **`SETUP-feed-pinger-cron-job-org.md`** (cron-job.org מפעיל את ה-workflow כל 5 דק' דרך API; דורש PAT + חשבון חינמי — 3 דק' פעולה של גל). קריאת ה-API אומתה (HTTP 204).
